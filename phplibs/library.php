...
// アップロードされたファイルを保存する
//##########################################################################################
function savefile($_file, $_dir) {
    global $_HOMEDIR_;

    $fileinfo = $_file[$_dir['key']];
    $tmpname = $fileinfo['tmp_name'];
    $type = $fileinfo['type'];
    $orgname = $fileinfo['name'];
    $ext = pathinfo($orgname, PATHINFO_EXTENSION);
    $savedir = $_HOMEDIR_."/".$_dir['savedir'];

    if (isset($_arg['filter'])) {
        $filter = $_arg['filter'];
    } else {
        $filter = array();
    }

    if (!in_array($ext, $filter) && count($filter) > 0) {
        return "{path:'', err:0, type:'".$type."'}";
    } else if(is_uploaded_file($tmpname)){
        $temppath = tempnam($savedir, "upload_");
        $savepath = $temppath;
        if ($ext != "") {
            $savepath .= (".".$ext);
        }
        ...
        move_uploaded_file($tmpname, $savepath);
        unlink($temppath);
        return "{path:'$fname', err:1, type:'$type', savedir:'$savedir', tmpname:'$tmpname'}";
    } else {
        return "{path:'', err:0, type:'".$type."'}";
    }
}
